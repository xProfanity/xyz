  ...
  return (
    <div className="flex flex-row flex-wrap justify-center items-center md:justify-normal md:items-start gap-4 w-full mt-4">
        ...
        {subjects?.map((subject, index) => (
          <Link href={`/student/lectures/${subject.subjectName?.toLowerCase()}`} key={index} className='w-[300px] rounded-lg bg-gray-300 hover:bg-gray-300/70 cursor-pointer flex flex-col justify-center items-start p-4'>
              <Image
                src={subject.picture}
                height={180}
                width={400}
